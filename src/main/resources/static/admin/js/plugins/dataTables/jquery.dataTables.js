...
(/** @lends <global> */function( $ ) {
	...
	var _pluck = function ( a, prop, prop2 ) {
		...
		// Could have the indexs.html in the loop for slightly smaller code, but speed
		// is essential here
		if ( prop2 !== undefined ) {
			for ( ; i<ien ; i++ ) {
				if ( a[i] && a[i][ prop ] ) {
	...
	function _fnGetObjectDataFn( mSource )
	{
			      ...
			      mSource.indexOf('[') !== -1 || mSource.indexOf('(') !== -1) )
		{
			 ...
			 * level down. On each loop we indexs.html for undefined, and if found immediately
